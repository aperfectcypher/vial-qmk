...
// Fill a rectangle on the OLED framebuffer
...
static void draw_logo(void) {
    ...
    //  Geometric "SLATE76" — 10px-wide chars, 3px stroke, 20px tall
    //  Centered in the 128×32 framebuffer with accent lines above/below
    const uint8_t Y  = 6;   // text top (vertically centered)
    const uint8_t S  = 3;   // stroke width
    const uint8_t H  = 20;  // character height
    const uint8_t W  = 10;  // character cell width
    const uint8_t G  = 3;   // gap between characters
    uint8_t x;
